...
function ReceiveOAuth() {
    ...
    if (searchParams.has("error")) {
        ...
        if (workerResponse.status === 200) {
            ...
            result = (
                <div>
                    Please don't attempt to use anything other than POST. Go back to <Link to="/onboard">the onboarding endpoint</Link>.
                </div>
            )
        } else {
            result = (
                <div>
                    Your access code is invalid. Please attempt <Link to="/onboard">reauthorization here</Link>.
                </div>
            )
        }
    } else {
